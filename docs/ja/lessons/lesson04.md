...
::: details ヒント
問題1の Pd パッチを拡張します。ボタン A の値（0/1）を `osc~ 500` の出力に `*~` で掛け合わせます。ボタン B も同様に `osc~ 1000` と組み合わせ、`output~` へ接続します。
:::
...
::: details ヒント
ボタンの分岐（`sel` や `osc.route`）の各アウトレットから、目標周波数と移行時間を組み合わせたメッセージ（例: `500 200`）を `line` を `osc~` の周波数インレットに接続しても、指定した時間をかけて周波数が滑らかに変化させることができます。
:::
